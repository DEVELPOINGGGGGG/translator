/* =======================================================
   AI PRO SUITE - THE ULTIMATE BUILD (V64 - COMPLETE RESTORE)
   Includes Synchronized Video Typewriter, Deep Search, Context Memory, Image Mode & Camera
======================================================= */
...
window.toggleImageModeBtn = function() {
    ...
    if (window.isImageGenerationMode) {
        if(btn) { btn.style.background = "linear-gradient(135deg, #f43f5e, #be123c)"; btn.innerHTML = "🖼️ Image Mode: ON"; }
        if(typeof showToast === 'function') showToast("🖼️ Image Generation Mode Enabled");
    } else {
        if(btn) { btn.style.background = "rgba(255,255,255,0.2)"; btn.innerHTML = "🖼️ Image Mode"; }
        if(typeof showToast === 'function') showToast("🖼️ Image Generation Mode Disabled");
    }
};
...
window.updateImageModeDisplay = function(direction = "") {
    ...
    if(indicator) indicator.innerText = `Page ${window.currentImagePage + 1} / ${window.imagePages.length}`;
    
    const prevBtn = document.getElementById("imgPrevBtn");
    const nextBtn = document.getElementById("imgNextBtn");
    if(prevBtn) prevBtn.style.display = window.currentImagePage > 0 ? "block" : "none";
    if(nextBtn) nextBtn.style.display = window.currentImagePage < window.imagePages.length - 1 ? "block" : "none";
};
...
window.nextImagePage = function(e) { 
    ...
    if(window.currentImagePage < window.imagePages.length - 1) { 
        ...
        window.updateImageModeDisplay("next"); 
    } 
};
...
window.prevImagePage = function(e) { 
    ...
    if(window.currentImagePage > 0) { 
        window.currentImagePage--; 
        window.updateImageModeDisplay("prev"); 
    } 
};
...
window.toggleImageControls = function() {
    ...
    if(controls) { controls.style.opacity = op; controls.style.pointerEvents = window.imageControlsVisible ? "auto" : "none"; }
    if(indicator) indicator.style.opacity = op;
    if(prev && prev.style.display !== "none") prev.style.opacity = op;
    if(next && next.style.display !== "none") next.style.opacity = op;
};
...
window.downloadImageMode = function() {
    if (typeof html2canvas === 'undefined') return showToast("❌ html2canvas is missing or blocked.");
...
[CRITICAL INSTRUCTION: At the very end of your ||MATH|| block, dynamically ask the user in your own words if they want you to generate another similar question for practice.]`;
    ...
    try {
        ...
        if (window.isImageGenerationMode || generateStandaloneShape) {
            ...
            let primaryColor = isDark ? '#8b5cf6' : '#3b82f6';
...
function deleteHistoryItem(e, id) { 
    e.stopPropagation(); 
    appHistory = appHistory.filter(i => String(i.id) !== String(id)); 
    saveHistorySafe(); 
    renderHistory(); 
...
window.restoreSession = function(e, id) { 
    if(e) e.stopPropagation(); 
    const item = appHistory.find(i => String(i.id) === String(id)); 
    if(!item) return; 
