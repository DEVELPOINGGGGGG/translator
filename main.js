/* =======================================================
   AI PRO SUITE - THE ULTIMATE BUILD (V63 - HISTORY ROUTER FIX)
...
function deleteHistoryItem(e, id) { 
    ...
    try {
        ...
        old = old.filter(i => String(i.id) !== String(id));
...
window.restoreSession = function(e, id) { 
    ...
    // REDIRECT TO THE PROPER PAGE IF WE AREN'T THERE YET
    if (targetPage !== '' && !currentPath.includes(targetPage)) { 
        window.location.href = `${targetPage}?restore=${id}`; 
        return; 
    }
    
    // IF WE ARE ON THE CORRECT PAGE, INJECT THE DOM
