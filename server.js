...
async function handleGeminiText(req, res) {
  try {
    ...
    const resultObj = await tryProviders(
      TEXT_PROVIDERS,
      async (p) => {
        if (p.type === "gemini") {
          ...
          const response = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${p.key}`,
          ...
          const response = await fetch(
            `https://api.cloudflare.com/client/v4/accounts/${p.accountId}/ai/v1/chat/completions`,
            {
              method: "POST",
...
// ==========================================
// VISION ENDPOINT (Perfect OCR / extract text from image)
// ==========================================
async function handleGeminiVision(req, res) {
  try {
    const body = JSON.parse(await readRequestBody(req));
    const img = body.imageBase64;
    const userText = body.userPrompt || body.prompt || body.text || "Describe the image and extract text.";
    const override = body.providerOverride || null;
...
// ==========================================
// IMAGE EXTRACT ENDPOINT (for frontend + button image -> Gemini -> refined query)
// ==========================================
async function handleImageExtract(req, res) {
  try {
    const body = JSON.parse(await readRequestBody(req));
    const img = body.imageBase64;
    if (!img || img === "data:,") return sendJson(res, 400, { error: "No image provided." });

    // Ask Gemini (or other TEXT_PROVIDERS) to extract and summarize key educational text from the image
    const prompt = `Extract and summarize the educational content from this image. Provide a concise search-ready YouTube query (India-focused) and a short title. Return JSON with keys: query, title, summary. Only return JSON.`;

    const resultObj = await tryProviders(
      TEXT_PROVIDERS,
      async (p) => {
        if (p.type === "gemini") {
          // Use Gemini vision-capable payload if available; otherwise send text prompt with image inlineData
          const rawBase64 = img.includes(",") ? img.substring(img.indexOf(",") + 1) : img;
          const payload = {
            contents: [
              {
                parts: [
    ...
    // Try to parse JSON from the model output; if parsing fails, return raw text as fallback
    let parsed = null;
    try {
      parsed = JSON.parse(resultObj.text);
    } catch (err) {
      // attempt to extract JSON substring
      const jsonMatch = resultObj.text.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        try {
    ...
    if (parsed) {
      return sendJson(res, 200, { parsed, raw: resultObj.text, provider: resultObj.provider });
    } else {
      // If model didn't return JSON, return the raw text and let frontend handle it
      return sendJson(res, 200, { parsed: null, raw: resultObj.text, provider: resultObj.provider });
    }
  } catch (e) {
    console.error("Image extract error:", e);
    return sendJson(res, 502, { error: e.message || "Image extract failed" });
  }
}
      ...
      async (p) => {
        if (p.type === "groq") {
          const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            method: "POST",
            headers: { Authorization: `Bearer ${p.key}`, "Content-Type": "application/json" },
            body: JSON.stringify({ model: "llama-3.3-70b-versatile", messages: [{ role: "system", content: sysText }, { role: "user", content: userText }] })
          });
          const data = await response.json();
          ...
          const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${p.accountId}/ai/v1/chat/completions`, {
            method: "POST",
            headers: { Authorization: `Bearer ${p.key}`, "Content-Type": "application/json" },
            body: JSON.stringify({ model: "@cf/meta/llama-3.1-70b-instruct", messages: [{ role: "system", content: sysText }, { role: "user", content: userText }] })
          });
          const data = await response.json();
...
// ==========================================
// YOUTUBE SEARCH ENDPOINT (Gemini -> yt-search) - returns up to 15 education-focused videos
// ==========================================
async function handleYoutubeSearch(req, res) {
  try {
    ...
    // Ask TEXT_PROVIDERS (Gemini preferred) to refine the search query into an education-focused query
    const refined = await tryProviders(
      TEXT_PROVIDERS,
      async (p) => {
        if (p.type === "gemini") {
          const payload = {
            contents: [
              {
                ...
                parts: [
                  {
                    text: `Refine this into a concise, India-focused YouTube search query for educational content only. Keep it short and search-ready: ${topic}`
                  }
                ]
              }
            ]
          ...
          const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${p.accountId}/ai/v1/chat/completions`, {
            ...
            body: JSON.stringify({
              model: "@cf/meta/llama-3.1-70b-instruct",
              messages: [
                { role: "system", content: "Refine the user topic into a short, India-focused YouTube search query for educational videos only." },
          ...
          const messages = [{ role: "user", content: `Refine this into a short India-focused YouTube search query for educational videos: ${topic}` }];
          const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            method: "POST",
            headers: { Authorization: `Bearer ${p.key}`, "Content-Type": "application/json" },
            body: JSON.stringify({ model: "llama-3.3-70b-versatile", messages })
    ...
    // Force education context to bias results toward tutorials/lectures
    const educationBoost = " tutorial lecture lesson explain how to course education";
    const finalQuery = `${query} India ${educationBoost}`;

    // Use yt-search to get results
    const search = await ytSearch(finalQuery);
    let videos = (search && search.videos) ? search.videos.slice(0, 30) : [];

    // Filter to educational keywords in title/description where possible
    const eduKeywords = ["tutorial", "lecture", "lesson", "explain", "how to", "course", "class", "education", "study", "guide", "introduction", "basics", "explanation"];
    const filtered = videos.filter((v) => {
      const text = `${v.title || ""} ${v.description || ""}`.toLowerCase();
      return eduKeywords.some((k) => text.includes(k));
    });

    // Prefer filtered list; if empty, fallback to original results
    videos = (filtered.length > 0) ? filtered : videos;

    // Limit to 15 results for frontend
    videos = videos.slice(0, 15);

    if (!videos || videos.length === 0) {
      return sendJson(res, 404, { error: "No educational videos found for this topic" });
    }
    ...
    const results = videos.map((v) => ({
      ...
      author: { name: (v.author && v.author.name) || v.author || "" },
      timestamp: v.timestamp || v.duration,
      seconds: v.seconds || 0,
      description: v.description || ""
    }));

    return sendJson(res, 200, { queryUsed: finalQuery, results, provider: refined.provider || "GEMINI" });
  } catch (e) {
...
function serveStatic(req, res) {
  let safePath = path
    .normalize(decodeURIComponent(new URL(req.url, `http://${req.headers.host}`).pathname))
    .replace(/^(\.\.[/\\])+/, "");
...
const server = http.createServer((req, res) => {
  if (req.method === "POST") {
    ...
    if (req.url === "/api/extract-from-image") return handleImageExtract(req, res);
  }
